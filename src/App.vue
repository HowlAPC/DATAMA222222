  ...
  <main v-else class="dashboard">
    <header>
      ...
      <div class="controls">
        ...
        <nav class="tabs">
          <button 
            v-for="tab in tabs.filter(t => userRole === 'admin' || t !== 'employees')"
            :key="tab"
      ...
      <table>
        ...
        <tbody>
          <tr v-for="row in filteredData"
              :key="row.customer_id || row.employee_id || row.item_id || row.receipt_id || row.payment_id">
...
.search-input { width: 100%; padding: 14px; margin-bottom: 20px; border: 1px solid #e2e8f0; border-radius: 10px; font-size: 1rem; }

.tabs { display: flex; gap: 8px; margin-bottom: 25px; border-bottom: 1px solid #e2e8f0; padding-bottom: 10px; overflow-x: auto; }
.tabs button { padding: 10px 18px; border: none; background: transparent; color: #64748b; cursor: pointer; border-radius: 6px; font-weight: 500; }
...
th { text-align: left; padding: 14px; background: #f8fafc; color: #475569; font-size: 0.85rem; text-transform: uppercase; }
