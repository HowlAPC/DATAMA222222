...
onMounted(async () => {
  ...
  if (user.value) {
    ...
    const { data: employeeData } = await supabase
      ...
      .eq('user_email', user.value.email) // make sure this column matches your table
      .single()
  ...
  <main v-else class="dashboard">
    <header>
      ...
      <div class="controls">
        ...
        />
        <nav class="tabs">
          <button v-for="tab in tabs" :key="tab" @click="activeTab=tab;searchQuery=''" :class="{active:activeTab===tab}">
            {{ tab.toUpperCase() }}
          </button>
        </nav>
      </div>
